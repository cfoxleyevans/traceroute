...
int main(int argc , char **argv) {
	...
	//loop for my max hops
   	for (ttl = 1; ttl < MAX_HOPS; ttl++) {
   		...
   		int i; //loop index 
   		//loop 3 times like the real traceroute
   		for (i  = 0; i < 3; i++) {
			...
			if (timeout_flag == 0) {
   				...
   				//if this is the first ping
   				if(ping_count == 0) { 
   					printf ("%i * ", hops);
   				}
   				//if the second or third ping
            	else {
   					printf("* ");
            	}
   			}
